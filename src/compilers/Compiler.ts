...
export const installer = new ResourcePacks({
	installGame: config.installToMinecraftPreview ? Minecraft.Preview : Minecraft.Stable,
	installFolder: config.installToDevelopEvironment ? ResourcePack.Development : ResourcePack.Production,
});
...
function manifestBuild(installPath: string) {
	...
	fs.writeFileSync(`${installPath}/manifest.json`, manifest.buildJson(), "utf-8");
}
...
process.on("beforeExit", () => {
	...
	try {
		...
		console.timeLog("Compiler", `>> _ui_defs.json ${UIBuilder.uiDefs(buildPath)} files path(s) found!`);
		console.timeLog("Compiler", `>> textures/textures_list.json ${UIBuilder.texturesList(buildPath)} files path(s) found!`);
		console.timeLog("Compiler", `>> contents.json ${UIBuilder.contents(buildPath)} file path(s) found!`);
		...
		console.log(`Pack Version: ${config.manifest.version[0]}.${config.manifest.version[1]}.${config.manifest.version[2]}`);
		console.log(`Base Gase Version: ${config.manifest.baseGameVersion[0]}.${config.manifest.baseGameVersion[1]}.${config.manifest.baseGameVersion[2]}`);
